...
class ProcessorDAG(object):
    ...
    def __init__(self, inputFilePath = None):
        ...
        if (inputFilePath == None):
            ...
            self.processors.append(Processor(16, False, Processor.CLOUD_TYPE_6))
        else:
    ...
    def getBandwidthToUse(self, fromProcessor, toProcessor):
        ...
        elif not fromProcessor.isFog and not toProcessor.isFog:
    ...
    def getTotalProcessingRate(self):
        ...
        for i in range(0, self.noOfFogs + self.noOfClouds):
            totalProcessingRate += self.processors[i].processingRate * self.processors[i].noOfCores

        return totalProcessingRate

    def getTotalNoOfCores(self):
        totalNoOfCores = 0

        for i in range(0, self.noOfFogs + self.noOfClouds):
            totalNoOfCores += self.processors[i].noOfCores

        return totalNoOfCores

    def getAvgProcessingRate(self):
        return self.getTotalProcessingRate() / self.getTotalNoOfCores()
    ...
    def getAvgBandwidth(self):
        return (self.getTotalDownloadBandwidth() + self.getTotalUploadBandwidth()) / (2 * (self.noOfClouds + self.noOfFogs))